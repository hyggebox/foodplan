...
@login_required(login_url='/')
def render_order_page_test(request):
    if request.method == "POST":
        current_user = User.objects.get(id=request.user.id)
        subs_interval, created = SubscriptionTimeInterval.objects.get_or_create(
            time_intervals=int(request.POST.getlist('subs_period')[0])
        ...
        if features_titles:
            features_titles = [feature_title for feature_title in
                               features_titles if feature_title]
...
@login_required(login_url='/')
def render_order_test_pay(request):
    if request.method == "POST":
        current_user = User.objects.get(id=request.user.id)
        subs_interval, created = SubscriptionTimeInterval.objects.get_or_create(
            time_intervals=int(request.POST.getlist('subs_period')[0])
        ...
        return render(request, 'order_test_pay.html', context)